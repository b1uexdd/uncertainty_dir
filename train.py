...
def get_checkpoint_state_dict(checkpoint):
    ...
    # Checkpoints created by the old train.py used DataParallel keys.
...
def load_model_state(model, checkpoint, strict=True):
    return unwrap_model(model).load_state_dict(
        get_checkpoint_state_dict(checkpoint),
        strict=strict,
    )
...
def main():
    ...
    # Stage 1 only trains the shared ResNet50 backbone and point regressor.
    # The two interval heads remain at their initialization for a later stage.
    for interval_head in (model.interval_lower, model.interval_upper):
        interval_head.requires_grad_(False)
        interval_head.eval()
    print('Stage 1: interval_lower and interval_upper are frozen')
    ...
    if args.pretrained:
        ...
        for k, v in get_checkpoint_state_dict(checkpoint).items():
            if ('linear' not in k and 'fc' not in k
                    and 'interval_lower' not in k
                    and 'interval_upper' not in k
                    and k in model_state
                    and model_state[k].shape == v.shape):
                new_state_dict[k] = v
        unwrap_model(model).load_state_dict(new_state_dict, strict=False)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            incompatible = load_model_state(model, checkpoint, strict=False)
            unexpected = list(incompatible.unexpected_keys)
            disallowed_missing = [
                key for key in incompatible.missing_keys
                if not key.startswith(('interval_lower.', 'interval_upper.'))
            ]
            if unexpected or disallowed_missing:
                raise RuntimeError(
                    'Checkpoint does not match ResNet50. '
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        save_checkpoint(args, {
            ...
            # Keep both names so train.py and train_ema_ver*.py can load it.
            # Saving the unwrapped model also avoids a "module." prefix.
...
def train(train_loader, model, optimizer, epoch):
    ...
    model.train()
    unwrap_model(model).interval_lower.eval()
    unwrap_model(model).interval_upper.eval()
    end = time.time()
